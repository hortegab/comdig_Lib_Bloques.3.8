...
class e_pdf2_ff(gr.sync_block):  # other base classes are basic_block, decim_block, interp_block
    """Hecho por Homero Ortega. Xmin is the minimum value that the input signal can take, Xmax is the maximum value that the input signal can take; N es the number of steps the signal will be analized in amplitud and is the number of samples for the histogram"""

    def __init__(self, Xmin=-1., Xmax=1., N=1024):  # only default arguments here
        gr.sync_block.__init__(
            self,
            name='e_pdf2_ff',   # will show up in GRC
            in_sig=[(np.float32, N)],
    ...
    def work(self, input_items, output_items):
        ...
        # cdf=hist_dist.cdf(self.X) # Este seria el comando si necesitamos cdf en vez de pdf
